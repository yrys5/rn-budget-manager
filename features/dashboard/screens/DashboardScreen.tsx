...
import { formatCurrency, getMonthLabel } from '@/shared/model/finance';
...
export default function DashboardScreen() {
  const { reload, state } = useDashboardSummary();

  if (state.status === 'loading' || state.status === 'idle') {
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.stateWrap}>
          <LoadingState title="Ładowanie pulpitu" />
        </View>
      </SafeAreaView>
    );
  }
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.stateWrap}>
          <ErrorState
            ...
            onRetry={reload}
            title="Nie udało się pobrać pulpitu"
          />
        </View>
      </SafeAreaView>
    );
  ...
  const {
    ...
    totalBalance,
    totalUsage,
  } = getDashboardMetrics(summary);
  const quickStats: { label: string; icon: IconName; value: string }[] = [
    { label: 'Budżety', icon: 'wallet-outline', value: `${summary.budgets.length}` },
    { label: 'Transakcje', icon: 'swap-horizontal-outline', value: `${summary.transactions.length}` },
    { label: 'Cele', icon: 'flag-outline', value: `${summary.goals.length}` },
    { label: 'Rodziny', icon: 'people-outline', value: `${summary.families.length}` },
  ];
  const limitInsights = summary.budgets
    .flatMap((budget) =>
      budget.limits.map((limit) => {
        const category = budget.categories.find((item) => item.id === limit.categoryId);
        const spent = category
          ? getCategorySpend(summary.transactions, budget.id, category, currentYear, currentMonth)
          : 0;
        const progress =
    ...
    .slice(0, 3);
  const latestTransactions = summary.transactions
    .slice()
    .sort((firstTransaction, secondTransaction) =>
      secondTransaction.transactionDate.localeCompare(firstTransaction.transactionDate),
    )
    .slice(0, 3);
  const nearestGoals = summary.goals
    .slice()
    .sort((firstGoal, secondGoal) => firstGoal.targetDate.localeCompare(secondGoal.targetDate))
    .slice(0, 2);
  const primaryFamily = summary.families[0];
  const primaryFamilyMembers = primaryFamily
    ? summary.familyMembers.filter((member) => member.familyId === primaryFamily.id)
    : [];
  const primaryFamilyBudgets = primaryFamily
    ? summary.familyBudgets.filter((familyBudget) => familyBudget.familyId === primaryFamily.id)
    : [];
  const familyMemberNames = primaryFamilyMembers
    .map((member) => summary.users.find((user) => user.id === member.userId)?.username)
        ...
        <View style={styles.section}>
          ...
          <View style={styles.listCard}>
            {latestTransactions.map((transaction) => {
              const { budget, category } = findCategory(summary.budgets, transaction);
